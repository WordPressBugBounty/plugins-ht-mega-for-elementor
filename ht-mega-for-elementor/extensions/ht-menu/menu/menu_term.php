...
class HTMega_Menu_Term_Meta{
     ...
     * hooked at wp_enqueue_scripts priority 20). That pass only registers is_singular()'s
     * queried post, so a template rendered via the nav walker never gets its atomic CSS
     * generated unless we register it here first.
     */
    ...
    public function custom_nav_fields_data( $menu_item ){
        ...
        // Icon
        if( !empty( $item_settings['menu-item-ficon-'.$menu_item->ID] ) ){
            $menu_item->ficon = $item_settings['menu-item-ficon-'.$menu_item->ID ];
        }
        if( !empty( $item_settings['menu-item-ficoncolor-'.$menu_item->ID] ) ){
            $menu_item->ficoncolor = $item_settings['menu-item-ficoncolor-'.$menu_item->ID];
        }

        // Badges
        if( !empty( $item_settings['menu-item-menutag-'.$menu_item->ID] ) ){
            $menu_item->menutag = $item_settings['menu-item-menutag-'.$menu_item->ID];
        }
        if( !empty( $item_settings['menu-item-menutagcolor-'.$menu_item->ID] ) ){
            $menu_item->menutagcolor = $item_settings['menu-item-menutagcolor-'.$menu_item->ID];
        }

        if( !empty( $item_settings['menu-item-badge-color-type-'.$menu_item->ID] ) ){
            $menu_item->badge_bg_type = $item_settings['menu-item-badge-color-type-'.$menu_item->ID];
        }
        if( !empty( $item_settings['menu-item-menutagbgcolor-'.$menu_item->ID] ) ){
            $menu_item->menutagbgcolor = $item_settings['menu-item-menutagbgcolor-'.$menu_item->ID];
        }
        if( !empty( $item_settings['menu-item-badge-bg-two-'.$menu_item->ID] ) ){
            $menu_item->badge_bg_color_two = $item_settings['menu-item-badge-bg-two-'.$menu_item->ID];
        }
