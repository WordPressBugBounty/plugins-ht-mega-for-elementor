...
class Bl_Page_Title_ELement extends Widget_Base {
    ...
    public function get_title() {
        return __( 'Page Title', 'ht-mega-for-elementor' );
    }
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section(
            'title_content',
            [
                'label' => __( 'Page Title', 'ht-mega-for-elementor' ),
            ]
        );
            $this->add_control(
                'title_html_tag',
    ...
    protected function render( $instance = [] ) {
        ...
        if( Elementor::instance()->editor->is_edit_mode() ){
            echo sprintf( '<%1$s class="htpage-title">' . esc_html__('Page Title', 'ht-mega-for-elementor' ). '</%1$s>', esc_attr( $title_tag ) );
        }else{
