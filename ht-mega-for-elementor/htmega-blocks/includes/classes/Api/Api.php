...
class Api {
	...
	public function register_routes() {
        ...
        // Template Library
        // \HtMegaBlocks\Template_Library::instance()->register_routes( $this->namespace );

        // CSS
        \HtMegaBlocks\Manage_Styles::instance()->register_routes( $this->namespace );
    ...
    public function get_category_data( $request ){
        ...
        $data = htmegaBlocks_taxnomy_data( $request['querySlug'], $request['queryLimit'], $request['queryOrder'], $request['queryType'] );
    ...
    public function get_image_sizes( $request ){
        ...
        $data = htmegaBlocks_get_image_size();
        return rest_ensure_response( $data );
    ...
    public function get_post_data( $request ){
        ...
        $data = [];
        $loop = new \WP_Query( htmegaBlocks_Product_Query( $request ) );
        ...
        if( $loop->have_posts() ){
            while( $loop->have_posts() ) {
                ...
                $item['badge']          = [
                    'sale_badge'        => htmega_sale_flash( 'default', false ),
                ];
                ...
                $item['rating']         = [
                    ...
                    'html2'             => htmega_wc_get_rating_html('yes'),
                ];
                ...
                $item['wishlist']       = [
                    'status' => htmega_has_wishlist_plugin(),
                    'html'   => htmega_add_to_wishlist_button()
                ];
                $item['compare']       = [
                    'status' => htmega_exist_compare_plugin(),
                    'html'   => htmegaBlocks_compare_button( array( 'style' => 2 ) ),
                    'html2'   => htmegaBlocks_compare_button( array( 'style' => 2, 'btn_text' => '<i class="fa fa-exchange"></i>', 'btn_added_txt' => '<i class="fa fa-exchange"></i>' ) ),
                ];
                ...
                if( has_post_thumbnail() ){
                    ...
                    $image_sizes    = htmegaBlocks_get_image_size();
                    $image_src      = array();
    ...
    public function get_last_product_data( $request ){
        ...
        if( !class_exists('\HtMega_Default_Data') ) {
            return [];
        }
        
        $product = \HtMega_Default_Data::instance()->get_product('');
        ...
        $item['addtocart']      = [
            ...
            'html'     => \HtMega_Default_Data::instance()->default('wl-product-add-to-cart')
        ];

        $item['price_html'] = \HtMega_Default_Data::instance()->default('wl-single-product-price');
        $item['short_description'] = \HtMega_Default_Data::instance()->default( 'wl-single-product-short-description' );
        $item['description'] = \HtMega_Default_Data::instance()->default( 'wl-single-product-description' );
        $item['rating'] = \HtMega_Default_Data::instance()->default( 'wl-single-product-rating' );
        $item['image'] = \HtMega_Default_Data::instance()->default( 'wl-single-product-image' );
        $item['meta_info'] = \HtMega_Default_Data::instance()->default( 'wl-single-product-meta' );
        $item['additional_info'] = \HtMega_Default_Data::instance()->default( 'wl-product-additional-information' );
        $item['product_tabs'] = \HtMega_Default_Data::instance()->default( 'wl-product-data-tabs2' );
        $item['product_reviews'] = \HtMega_Default_Data::instance()->default( 'wl-single-product-reviews' );
        $item['product_stock'] = \HtMega_Default_Data::instance()->default( 'wl-single-product-stock' );
        $item['product_upsell'] = \HtMega_Default_Data::instance()->default( 'wl-single-product-upsell' );
        $item['product_related'] = \HtMega_Default_Data::instance()->default( 'wl-product-related', array('orderby'=>'date','order'=>'desc') );
